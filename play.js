...
function loadHistory() {
  ...
  history.forEach((movie) => {
    ...
    historyGrid.innerHTML += `
      <div class="history-item" onclick="location.href='./play.html?id=${
        movie.id
...
function loadMovieDetails() {
  ...
  searchListMovies.forEach((movie) => {
    movie.addEventListener("click", () => {
      ...
      const url = `./play.html?id=${imdb_id}`;
...
async function displayMovieDetails(imdb_id) {
  ...
  if (details.Type == "movie") {
    ...
    const playButton = document.getElementById("watch-movie");
    playButton.click();}
    });
  } else {
...
async function displayVideo(episodeId, mediaId) {
  ...
  videoGrid.innerHTML = `<div class="data-vjs-player"><video id="video_1" class="video-js" autoplay="true" width="100%" preload="auto" src=""></video><br></br>
